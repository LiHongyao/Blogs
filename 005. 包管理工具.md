...
1. npm（**N**ode **P**ackage **M**anager）：最常用的包管理工具，它是Node.js的默认包管理器，npm具有强大的功能和广泛的生态系统。它使用`package.json`文件来管理项目的依赖关系和脚本。
2. Yarn：由Facebook开发的包管理工具，旨在解决npm的一些性能和安全性问题。Yarn 支持 **离线安装** 和 **并行下载** 等功能。
...
npm（**N**ode **P**ackage **M**anager，Node 包管理器）是 nodeJS 的包管理工具，随同 nodeJS 一起安装。其能解决 nodeJS 代码部署上的很多问题，常见的使用场景有以下几种：
...
- 节省磁盘空间
  ...
  - 所有文件都保存在硬盘上的统一的位置。当安装软件包时， 其包含的所有文件都会硬链接自此位置，而不会占用 额外的硬盘空间，实现依赖共享。

- 提升安装速度

  - pnpm：依赖性解析 →  目录结构计算 → 链接相关性
  - 传统方式：解析 → 获取 → 将所有依赖写入 node_modules
