...
OR
$ npm i -D webpack webpack-command
```
...
> 提示：想了解webpack更多参数，可在终端输入 ./node_modules/.bin/webpack  --help`  查看
...
```json
"scripts": {
    "build": "./node_modules/.bin/webpack --config ./build/webpack.dev.config.js -w"
}
```
...
要使用某个插件，我们需要通过 NPM 安装它，然后要做的就是在webpack配置中的**plugins**关键字部分添加该插件的一个实例（plugins是一个数组）。
...
[参考](http://www.cnblogs.com/wonyun/p/6030090.html)
...
1. 一旦修改了webpack的配置文件，必须重启服务或重新build。否则失效。
