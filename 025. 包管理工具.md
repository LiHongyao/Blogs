...
NPM（Node Package Manager，即：node包管理器）是nodeJS的包管理工具，NPM 完全用 **JavaScript** 写成。NPM 是随同 NodeJS 一起安装的包管理工具，能解决NodeJS代码部署上的很多问题，常见的使用场景有以下几种：
...
2. 在用户目录下建立一个新的目录，并设置为npm全局的prefix
