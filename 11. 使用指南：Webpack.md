...
首先创建一个空的项目（*webpack-test*），打开终端，`cd` 到项目中，输入如下指令创建 *"package.json"*。
...
Loaders和Plugins常常被弄混，但是他们其实是完全不同的东西，可以这么来说，loaders是在打包构建过程中用来处理源文件的（Scss，Less等），一次处理一个，插件并不直接操作单个文件，它直接对整个构建过程其作用。

Webpack有很多内置插件，同时也有很多第三方插件，可以让我们完成更加丰富的功能。

要使用某个插件，我们需要通过 `npm` 安装它，然后要做的就是在webpack配置中的plugins关键字部分添加该插件的一个实例（plugins是一个数组）。

## 1. [BannerPlugin](https://webpack.js.org/plugins/banner-plugin/)
...
## 2. [HtmlWebpackPlugin](https://github.com/ampedandwired/html-webpack-plugin)
...
## 4. [clean-webpack-plugin](https://www.npmjs.com/package/clean-webpack-plugin)
...
执行指令 `npm run webpack`，然后修改源文件再执行，多次操作，观察 *./dist/* 目录，如下所示：

```
dist
├── app.bundle.e56abf8d6e5742c78c4b.js
├── bundle.e81bf3def4b4f81ea5a1.js
...
├── index.html
└── style.css
```

生成的带 hash 的 `bundle.js` 文件就会很多。这些带 hash 的 `bundle.js` 只有最新的才有用，其他的都没用，我们要在 build 之前把它们全清空，这就是 clean-webpack-plugin 发挥的作用。
...
执行 `npm run webpack`，*“./dist”* 目录下之前生产的文件已被删除。
...
> 提示：
>
> 示例中 `excludeChunks` 表示不包含，`chunks` 表示包含。
