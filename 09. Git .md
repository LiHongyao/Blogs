...
1. 组长添加组员SSH Key。注意：组员SSH Key如果在自己账号中已经使用，先删除

2. 组长创建远程仓库，并且添加dev分支

3. 克隆仓库

4. 组员在 ”.git/config“文件的 ”[remote "origin"]“ 字段下，修改url字段，如下所示：

   ```properties
   url = https://用户名:密码@github.com/用户名/仓库名.git
   ```

5. 创建dev分支：`git checkout -b dev origin/dev`

6. 开发业务逻辑

7. 提交/推送分支：`git push origin dev`
