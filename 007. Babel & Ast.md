...
这里一定要注意， `path` 和前面说的 `AST` 节点 不是一个概念，`path` 表示两个节点之间 **连接** 的 **对象**，通常我们编写插件，主要也是在通过它来修改 AST 语法树的。

![](./IMGS/babel_path.png)

上图中红色方框处表示对应节点的 path类型。

属性：

- `state`：Babel 插件信息，可通过 `state.opts` 获取传入的 Options；
- `node`：当前遍历到的 node 节点，可通过它访问节点上的属性，对于 Ast 节点；
- `parent`：父级 node，无法进行替换；
- `parentPath`：父级 path，可进行替换；
- `scope`：作用域相关，可用于变量重命名，变量绑定关系检测等；

方法：

- `path.toString()`：当前路径所对应的源代码。
- `path.isXXX`：判断 path 类型，比如我们需要判断路径是否为 `StringLiteral` 类型 → `path.isStringLiteral`
